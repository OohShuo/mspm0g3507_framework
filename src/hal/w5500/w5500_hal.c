#include "w5500_hal.h"
    ...
    #define W5500_SPI_CHUNK_SIZE 512u
...
static void cs_sel(void) { Bsp_Gpio_Write(g_wiz->config.cs_gpio_idx, bsp_gpio_state_reset); }
static void cs_desel(void) { Bsp_Gpio_Write(g_wiz->config.cs_gpio_idx, bsp_gpio_state_set); }
...
static void spi_wb(uint8_t wb) { Bsp_Hard_Spi_Write_Blocking(g_wiz->config.spi_idx, &wb, 1); }

static void spi_rburst(uint8_t* buf, uint16_t len) {
    while (len > 0u) {
        uint16_t chunk = len > W5500_SPI_CHUNK_SIZE ? W5500_SPI_CHUNK_SIZE : len;
        Bsp_Hard_Spi_Read_Blocking(g_wiz->config.spi_idx, buf, chunk);
        buf += chunk;
        len -= chunk;
    }
}
static void spi_wburst(uint8_t* buf, uint16_t len) {
    while (len > 0u) {
        uint16_t chunk = len > W5500_SPI_CHUNK_SIZE ? W5500_SPI_CHUNK_SIZE : len;
        Bsp_Hard_Spi_Write_Blocking(g_wiz->config.spi_idx, buf, chunk);
        buf += chunk;
        len -= chunk;
    }
}
...
static void cris_en(void) {
    if (g_wiz->config.spi_mutex) xSemaphoreTake(g_wiz->config.spi_mutex, portMAX_DELAY);
}
static void cris_ex(void) {
    if (g_wiz->config.spi_mutex) xSemaphoreGive(g_wiz->config.spi_mutex);
}
...
Wiz5500* W5500_Create(const W5500_Config* cfg) {
    ...
    if (cfg->spi_idx >= SPI_NUM || cfg->cs_gpio_idx >= GPIO_NUM) return NULL;
    if (cfg->rst_gpio_idx != UINT32_MAX && cfg->rst_gpio_idx >= GPIO_NUM) return NULL;
...
SemaphoreHandle_t W5500_Get_Mutex(Wiz5500* obj) { return (obj != NULL) ? obj->config.spi_mutex : NULL; }
