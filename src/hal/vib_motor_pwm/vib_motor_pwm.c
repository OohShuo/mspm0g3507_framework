...
static Vector* vib_motor_pwm_instances;
...
static const Vib_motor_pwm_step pattern_life_lost_steps[] = {{45u, 80u, 40u}, {35u, 80u, 0u}};
static const Vib_motor_pwm_pattern pattern_life_lost = {pattern_life_lost_steps, 2u};
static const Vib_motor_pwm_step pattern_victory_steps[] = {{30u, 40u, 50u}, {30u, 40u, 0u}};
static const Vib_motor_pwm_pattern pattern_victory = {pattern_victory_steps, 2u};

static const Vib_motor_pwm_pattern* const effect_patterns[vib_pwm_effect_count] = {&pattern_menu_tick,
...
static void finish_playback(Vib_motor_pwm* obj) {
    obj->active = 0u;
    obj->priority = 0u;
    obj->pattern = NULL;
    obj->step_index = 0u;
...
static void start_pattern_step(Vib_motor_pwm* obj) {
    const Vib_motor_pwm_step* step = &obj->pattern->steps[obj->step_index];
...
static void update_pattern(Vib_motor_pwm* obj, uint32_t now) {
...
Vib_motor_pwm* Vib_Motor_Pwm_Create(const Vib_motor_pwm_config* config) {
    if (config == NULL || vib_motor_pwm_instances == NULL) { return NULL; }
    Vib_motor_pwm* obj = (Vib_motor_pwm*)pvPortMalloc(sizeof(*obj));
    if (obj == NULL) { return NULL; }
    memset(obj, 0, sizeof(*obj));
    obj->config = *config;
    obj->config.pwm_freq_hz = config->pwm_freq_hz == 0u ? VIB_MOTOR_PWM_DEFAULT_FREQ_HZ : config->pwm_freq_hz;
    ...
    obj->enabled = 1u;
    Vector_Push_Back(vib_motor_pwm_instances, (void*)&obj);
    return obj;
}
...
void Vib_Motor_Pwm_Play_Effect(Vib_motor_pwm* obj, Vib_motor_pwm_effect effect) {
    if (obj == NULL || (unsigned)effect >= vib_pwm_effect_count) { return; }
    taskENTER_CRITICAL();
    const uint32_t now = Bsp_Get_Tick_Ms();
...
void Vib_Motor_Pwm_Update_All(void) {
    if (vib_motor_pwm_instances == NULL) { return; }
    const uint32_t now = Bsp_Get_Tick_Ms();
    taskENTER_CRITICAL();
    for (uint32_t i = 0; i < Vector_Get_Size(vib_motor_pwm_instances); i++) {
        Vib_motor_pwm* obj = *(Vib_motor_pwm**)Vector_Get_At(vib_motor_pwm_instances, i);
...
uint8_t Vib_Motor_Pwm_Is_Enabled(Vib_motor_pwm* obj) { return obj == NULL ? 0u : obj->enabled; }
