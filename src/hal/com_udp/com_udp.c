...
Com_udp* Com_Udp_Create(const Com_udp_config* cfg) {
    if (cfg == NULL || g_instances == NULL) return NULL;
    if (cfg->wiz == NULL) return NULL;
    if (cfg->rx_buf_size == 0 || cfg->tx_buf_size == 0) return NULL;
    ...
    if (obj->rx_buf == NULL) { vPortFree(obj); return NULL; }

    obj->tx_buf = (uint8_t*)pvPortMalloc(cfg->tx_buf_size);
    if (obj->tx_buf == NULL) { vPortFree(obj->rx_buf); vPortFree(obj); return NULL; }

    int8_t ret = socket(cfg->sock_n, Sn_MR_UDP, cfg->local_port, 0);
    if (ret != (int8_t)cfg->sock_n) {
        vPortFree(obj->tx_buf); vPortFree(obj->rx_buf); vPortFree(obj);
        return NULL;
    }
...
void Com_Udp_Poll(void) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        case SOCK_UDP: {
            ...
            if (avail == 0) break;
            if (avail > obj->config.rx_buf_size)
                avail = obj->config.rx_buf_size;

            int32_t rlen = recvfrom(sn, obj->rx_buf, avail,
                                    obj->last_src_ip, &obj->last_src_port);
            if (rlen <= 0) break;

            if (obj->config.on_rx) {
                obj->config.on_rx(obj, obj->rx_buf, (uint32_t)rlen,
                                  PROTOCOL_CHUNK_FIRST | PROTOCOL_CHUNK_LAST,
                                  obj->config.on_rx_arg);
            }
            break;
        }
        case SOCK_CLOSED:
...
void Com_Udp_Send(Com_udp* obj, const uint8_t* data, uint32_t len,
    ...
    if (obj == NULL || data == NULL || len == 0) return;
    if (len > obj->config.tx_buf_size) return;
    if (getSn_SR(obj->config.sock_n) != SOCK_UDP) return;

    memcpy(obj->tx_buf, data, len);
    sendto(obj->config.sock_n, obj->tx_buf, (uint16_t)len,
           (uint8_t*)dest_ip, dest_port);
}
