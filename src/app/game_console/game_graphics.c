...
void Game_Graphics_Draw_Pal4_Bitmap(
    ...
    for (int32_t row = 0; row < h; row++) {
        ...
        while (remaining >= 2) {
            const uint8_t byte = *src++;
            *dst++ = palette[byte >> 4];    /* high nibble → first pixel */
            *dst++ = palette[byte & 0x0Fu]; /* low nibble  → second pixel */
            remaining -= 2;
        }
        if (remaining > 0) {
            /* Odd-width image: last byte has only one valid pixel (high nibble) */
...
void Game_Graphics_Draw_Top_Bar(St7789* lcd, const char* game_name) {
    ...
    if (game_name != NULL) { Game_Graphics_Draw_Text(lcd, 8, 3, game_name, 3, 0xffffu); }
...
void Game_Graphics_Draw_Bottom_Bar(St7789* lcd, const char* hint_text, uint16_t fps) {
    ...
    Game_Graphics_Fill_Rect(
        lcd, 0, GAME_AREA_BOTTOM + 1, SCREEN_WIDTH, GAME_BOTTOM_BAR_H - 1, GAME_BAR_COLOR_BG);
