...
static void draw_hud(void) {
    /* Combo + lives + score — right-aligned in top bar, scale=1 */
    Game_Graphics_Fill_Rect(g_hardware.lcd, 108, 4, 128, 8, GAME_BAR_COLOR_BG);
    Game_Graphics_Draw_Text(g_hardware.lcd, 110, 4, "C:", 1, COLOR_GRAY);
    Game_Graphics_Draw_U32(g_hardware.lcd, 126, 4, g_combo, 3, 1, COLOR_CYAN);
    Game_Graphics_Draw_Text(g_hardware.lcd, 152, 4, "L:", 1, COLOR_GRAY);
    Game_Graphics_Draw_U32(g_hardware.lcd, 166, 4, g_lives, 1, 1, COLOR_RED);
    Game_Graphics_Draw_U32(g_hardware.lcd, 202, 4, g_score, 5, 1, COLOR_WHITE);
}

static void draw_mode(void) {
    fill(84, 120, 72, 10, COLOR_BLACK);
    Game_Graphics_Draw_Text(g_hardware.lcd, 86, 120, g_song_mode ? "LOOP" : "RAND", 1, COLOR_YELLOW);
}

static void draw_feedback(void) {
    fill(36, 58, 168, 30, COLOR_BLACK);
    if (g_feedback_frames == 0u) { return; }
    if (g_feedback_kind == 1u) {
        Game_Graphics_Draw_Text(g_hardware.lcd, 78, 62, "PERFECT", 2, COLOR_CYAN);
    } else if (g_feedback_kind == 2u) {
        Game_Graphics_Draw_Text(g_hardware.lcd, 96, 62, "GOOD", 2, COLOR_GREEN);
    } else if (g_feedback_kind == 3u) {
        Game_Graphics_Draw_Text(g_hardware.lcd, 96, 62, "MISS", 2, COLOR_RED);
    } else {
        Game_Graphics_Draw_Text(g_hardware.lcd, 54, 62, "COMBO", 2, COLOR_YELLOW);
        Game_Graphics_Draw_U32(g_hardware.lcd, 130, 62, g_combo, 3, 2, COLOR_WHITE);
    }
    g_feedback_frames--;
}
