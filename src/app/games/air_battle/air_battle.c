...
static void air_battle_draw_icon(St7789* lcd, int32_t x, int32_t y) {
    x += 2;
    y += 1;
    Game_Graphics_Fill_Rect(lcd, x + 20, y, 7, 35, 0x07ffu);
    Game_Graphics_Fill_Rect(lcd, x + 12, y + 10, 23, 15, 0x0010u);
    Game_Graphics_Fill_Rect(lcd, x + 4, y + 17, 39, 7, 0x07ffu);
    Game_Graphics_Fill_Rect(lcd, x + 16, y + 27, 15, 5, 0xffffu);
    Game_Graphics_Fill_Rect(lcd, x + 22, y - 3, 3, 7, 0xffe0u);
}

const Game_descriptor game_air_battle_entry = {
    .draw_icon = air_battle_draw_icon,
    .name_color = 0x07ffu,
    .name = "AIR FORCE",
    .id = game_id_air_battle,
    .control_hint = "A BOMB",
    .info_text =
