...
static const Vib_motor_pwm_step g_life_lost_steps[] = {{45u, 80u, 40u}, {35u, 80u, 0u}};
static const Vib_motor_pwm_pattern g_life_lost = {g_life_lost_steps, 2u};
static const Vib_motor_pwm_step g_victory_steps[] = {{30u, 40u, 50u}, {30u, 40u, 0u}};
static const Vib_motor_pwm_pattern g_victory = {g_victory_steps, 2u};

static const Vib_motor_pwm_pattern* const g_effect_patterns[vib_pwm_effect_count] = {&pattern_menu_tick,
    ...
    &pattern_defeat};
static const uint8_t g_effect_priorities[vib_pwm_effect_count] = {
...
static void stop_output(Vib_motor_pwm* motor) {
    motor->output_on = 0u;
    Vm_Haptics_Set_Strength(0u);
}

static void output_strength(Vib_motor_pwm* motor, uint8_t requested_strength) {
...
static uint8_t can_accept(Vib_motor_pwm* motor, uint8_t priority, uint32_t now) {
    if (!motor->enabled || (motor->active && priority < motor->priority)) { return 0u; }
    if (motor->has_played && now - motor->last_play_at < VIB_MOTOR_PWM_MIN_RETRIGGER_MS) { return 0u; }
    motor->has_played = 1u;
...
static void finish_playback(Vib_motor_pwm* motor) {
    motor->active = 0u;
    motor->priority = 0u;
    motor->pattern = NULL;
    motor->step_index = 0u;
...
static void start_pattern_step(Vib_motor_pwm* motor) {
    const Vib_motor_pwm_step* step = &motor->pattern->steps[motor->step_index];
...
static void update_pattern(Vib_motor_pwm* motor, uint32_t now) {
...
void Vib_Motor_Pwm_Play_Effect(Vib_motor_pwm* motor, Vib_motor_pwm_effect effect) {
    if (motor == NULL || (unsigned)effect >= vib_pwm_effect_count) { return; }
    taskENTER_CRITICAL();
    const uint32_t now = Bsp_Get_Tick_Ms();
