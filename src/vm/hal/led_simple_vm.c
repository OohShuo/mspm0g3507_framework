...
void Led_Simple_Set_State(Led_simple* o, Led_simple_state s) {
    if (o) o->state = s;
}
void Led_Simple_Toggle(Led_simple* o) {
    if (o) o->state = (o->state == led_simple_state_on) ? led_simple_state_off : led_simple_state_on;
}
void Led_Simple_Set_Blink_Freq(Led_simple* o, uint8_t f) {
    if (o) o->blink_freq_hz = f;
}
