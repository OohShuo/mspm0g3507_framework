...
int lfs_getattr(lfs_t* lfs, const char* path, uint8_t type, void* buf, lfs_size_t size) {
    (void)lfs; (void)path; (void)type; (void)buf; (void)size;
    return LFS_ERR_INVAL;
}

/* ── File operations ── */

static const char* fopen_mode_for_flags(int flags) {
    int rd = flags & 3;
    if (rd == LFS_O_RDONLY) { return "rb"; }
    if (rd == LFS_O_WRONLY) { return "r+b"; }
    return "r+b"; /* RDWR */
}
...
int lfs_file_open(lfs_t* lfs, lfs_file_t* file, const char* path, int flags) {
    ...
    if ((flags & LFS_O_CREAT) && (flags & LFS_O_EXCL) && exists) {
...
lfs_soff_t lfs_file_seek(lfs_t* lfs, lfs_file_t* file, lfs_soff_t off, int whence) {
    ...
    int w = SEEK_SET;
    if (whence == LFS_SEEK_CUR) { w = SEEK_CUR; }
    else if (whence == LFS_SEEK_END) { w = SEEK_END; }
...
int lfs_dir_open(lfs_t* lfs, lfs_dir_t* dir, const char* path) {
    ...
    if (strcmp(fd.cFileName, ".") != 0 && strcmp(fd.cFileName, "..") != 0) {
        dir->current.type = (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
