...
void Vm_Display_Init(void) {
    ...
    g_window = SDL_CreateWindow("Game Console VM [240x320 @2x]",
        SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
        480, 640, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    if (!g_window) return;
    ...
    g_texture = SDL_CreateTexture(g_renderer, SDL_PIXELFORMAT_RGB565,
        SDL_TEXTUREACCESS_STREAMING, 240, 320);
    if (!g_texture) return;
...
void Vm_Display_Write_Pixels(int16_t x1, int16_t y1, int16_t x2, int16_t y2,
    ...
    for (int16_t y = y1; y <= y2 && idx < pixel_count; y++)
        for (int16_t x = x1; x <= x2 && idx < pixel_count; x++)
            g_fb[y][x] = px[idx++];
}
