...
static Soft_crc16 s_crc16_default;
static Soft_crc8 s_crc8_default;
Soft_crc16* soft_crc16_default = &s_crc16_default;
Soft_crc8* soft_crc8_default = &s_crc8_default;
...
void Soft_Crc_Init(void) {
    ...
    // Fill the static tables in place. Living in BSS, not the heap, so
    // a stray DMA write or a corrupted heap_4 free-list cannot turn
    // soft_crc16_default into a NULL/wild pointer mid-run.
    Soft_crc16_config crc16_cfg = {
        .poly = SOFT_CRC16_CCITT_POLY,
        .crc_init = SOFT_CRC16_MODBUS_INIT,
    };
    Soft_crc16 tmp16 = {0};
    uint16_t table_poly = bit_reverse_u16(crc16_cfg.poly);
    for (uint32_t i = 0; i < 256; i++) {
        uint16_t crc = 0;
        uint16_t c = (uint16_t)i;
        for (uint32_t j = 0; j < 8; j++) {
    ...
    tmp16.crc_init = crc16_cfg.crc_init;
    s_crc16_default = tmp16;
    ...
    Soft_crc8 tmp8 = {0};
    uint8_t table_poly8 = bit_reverse_u8(crc8_cfg.poly);
    for (uint32_t i = 0; i < 256; i++) {
        uint8_t crc = 0;
        uint8_t c = (uint8_t)i;
        for (uint32_t j = 0; j < 8; j++) {
            if ((crc ^ c) & 0x01u) {
                crc = (uint8_t)((crc >> 1) ^ table_poly8);
            } else {
                crc = (uint8_t)(crc >> 1);
            }
            c = (uint8_t)(c >> 1);
    ...
    tmp8.crc_init = crc8_cfg.crc_init;
    s_crc8_default = tmp8;
}
